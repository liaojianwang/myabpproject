    ...
    $.ajax({
        ...
        success: function (data, textStatus) {
            ...
            if (data) {
                ...
                //    var row = data[i];
                //    if (row.parentId == 0) {
                //        topHtml += '<div class=\"list-group\">';
                //        topHtml += '<h1 title="' + row.navTitle + '">';
                //        if (row.iconUrl) {
                //            if (row.iconUrl.indexOf('.') > -1) {
                //                topHtml += '<i class="iconfont ' + row.iconUrl + '"></i>';
                //            } else {
                //                topHtml += '<img src="' + row.iconUrl + '" />';
                //            }
                //        }
                //        topHtml += '</h1>';
                //        topHtml += '<div class="list-wrap">';
                //        if (row.navTitle)
                //            topHtml += '<h2>' + row.navTitle + '<i class="iconfont icon-arrow-down"></i></h2>';
                //        topHtml += '</div>';
                //        topHtml += '</div>';
                //    }
                //}
                topHtml += getNavChilds(topHtml, data, 0);
                console.log(topHtml);
            }
            //将得到的数据插件到页面中
            //$("#sidebar-nav").html(data);
            $("#sidebar-nav").html(topHtml);
...
function getNavChilds(html, data, parentId) {
    ...
    for (var i = 0; i < query.length; i++) {
        ...
        if (row.parentId == 0) {
            ...
            html += '<h1 title="' + row.navTitle + '">';
            if (row.iconUrl) {
                if (row.iconUrl.indexOf('.') > -1) {
                    html += '<i class="iconfont ' + row.iconUrl + '"></i>';
                } else {
            ...
            if (row.navTitle)
                html += '<h2>' + row.navTitle + '<i class="iconfont icon-arrow-down"></i></h2>';
            html +=  getNavChilds(html, data, row.id)
            html += '</div>';
            html += '</div>';
        }
        else {
            html += '<ul>';
            html += '<li>';
            html += '<a navid="' + row.navName + '"';
            if (row.linkUrl) {
                if (row.channelId > 0) {
                    html += ' href="' + row.linkUrl + '?channel_id=' + row.channelId + '" target="mainframe"';
                } else {
                    html += ' href="' + row.linkUrl + '" target="mainframe"';
                }
            }
            html += ' icon="' + row.iconUrl + '"';
            html += ' target="mainframe">';
            html += '<span>' + row.navTitle + '</span>';
            html += '</a>';
            html += getNavChilds(html, data, row.id);
            html += '</li>';
            html += '</ul>';
